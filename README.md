...
Image caption node for ComfyUI. You can load your image caption model and
generate prompts with the given picture.
...
- You can find the `Prompt Generator` node in
  [this repository](https://github.com/alpertunga-bile/prompt-generator-comfyui)
...
- For to use the pretrained model follow these steps:
  ...
  - Refresh or restart the ComfyUI
  - Then select the image caption model with the node's `model_name` variable
...
- [x] Increasing the `min_new_tokens` and `max_new_tokens` variables' values
      will help to generate more accurate prompts.
...
- For string output; additional comma is added for compability with other nodes
  which are appending new prompts for the given string, for example
  `Prompt Generator`.
...
- The image caption node is based on
  [transformers](https://github.com/huggingface/transformers) package. So most
  of the problems may be caused from this package. For overcome these problems
  you can try to update package:
...
- For Portable Installation of the ComfyUI
  ...
  3. Run the
     `.\python_embeded\python.exe -s -m pip install --upgrade transformers`
     command.
